            ...
            for file_name in files:
                if file_name.endswith('.json'):
                    ...
                    with open(file_path, 'r') as file:
                        data = json.load(file)
                        button = QPushButton(data['name'])
                        button.clicked.connect(lambda _, file_path=file_path: create_pet(file_path))  # 传递file_path参数
                        layout.addWidget(button)
        self.setLayout(layout)
...
def create_main_ui():
    ...
    button1.clicked.connect(show_pet_list_dialog)
